...
jQuery(function($) {
    ...
    // Listen for repeat confirmation...
    $('input[name="repeat_customer"]').change(function(e) {
        if($('#repeatCustomer-f').is(':checked')) {
            $('#bluSurvey2').fadeOut();
            setTimeout(function() {
                $('#bluSurvey').fadeIn();
            }, 0);
            console.log('should be fading out');
        } else if($('#repeatCustomer-t').is(':checked')) {
            $('section#revisitConfirmation').fadeOut();
            setTimeout(function() {
                $('section#sampleFeedback').fadeIn();
            }, 800);
        }
    });

    // Listen for 'Why didn't you like the sample' response changes...
    $('input[name="liked_sample"]').change(function(e) {
        likedSample = $('input[name=liked_sample]:checked', '#bluSurvey2').val();

        if (likedSample == 'No') {
            $('#dislikedContainer').fadeIn();
        } else if (likedSample == 'Yes') {
            $('#dislikedContainer').fadeOut();
            // reset the previously hidden field
            $('textarea[name="liked_no_reason"]').val('');
        }
    });
    ...
    $('#startSurvey').click(function() {
        if (validateForm()) {
            if (identifyMatch()) {
                ...
                // Get the last sample
                var samplesTried = identityResult.products.length;
                console.log(samplesTried);
                $('span#populatedSample').html(identityResult.products[samplesTried - 1]);
                $('#initialID').val(identityResult.id);
            } else {
                $('#bluSurvey').fadeIn();
                prevSurveySection = 'form';
            }
    ...
    // Repeat Customer
    $('#nextRepeat1').click(function() {
        currentSurveySection = 'repeat1';
        goNext(currentSurveySection);
    });
    ...
    // Repeat Questions
    $('#prevRepeat1').click(function() {
        prevSurveySection = 'form';
        goPrevious(prevSurveySection);
    });

    $('#prevFinal2').click(function() {
        goPrevious(prevSurveySection);
    });
...
$('#submit').click(function (event) {
    ...
    } else if (sampleValid && kitValid && couponValid) {
...
$('#submit2').click(function (event) {
    var sampleValid = $('input:radio[name="sampling_flavor_2"]').is(':checked'),
        // kitValid = $('input:radio[name="sampling_kit_2"]').is(':checked'),
        couponValid;
    event.preventDefault();

    // Check if coupon is valid
    if ($('#couponCode2').val().length === 5) {
        couponValid = true;
    } else {
        couponValid = false
        $('#couponCode2').addClass('warn');
    }
    ...
    // } else if (!kitValid) {
    //     alert('Did you give away a kit?')
    } else if (sampleValid && couponValid) {
        // ninja insert the date in the correct format
        $('#bluForm-dob').val(reformattedDOB);
        // disable button after being clicked
        ...
        $.ajax({
            type: "POST",
            url: "http://blu.momentum.clong.mmahost5.networkninja.com/api/v1/save",
            dataType: "json",
            data: {data : formData},
            success: function(result) {
                submissionBacklog = window.localStorage.getArray('storedSubmissions');
...
function goNext(currentPage) {
    ...
    switch (currentPage) {
        ...
        case 'repeat1':
            console.log(currentPage);
            if (!$('input[name="liked_sample"]:checked').val()) {
                alert('Did you like your sample?');
            } else if (($('input[name="liked_sample"]:checked').val() == 'No') && ($('textarea[name="liked_no_reason"]').val() == '')) {
                alert('Why didn\'t you like your sample?');
            } else {
                $('section#sampleFeedback').fadeOut();
                setTimeout(function() {
                    $('section#finalRepeat').fadeIn();
                    currentSurveySection = 'final2';
                }, 800);

                prevSurveySection = 'repeat1';
            }
...
function goPrevious(previousPage) {
    ...
    switch (previousPage) {
        ...
        case 'start':
            ...
            $('section#repeat1').fadeOut();
            $('input[name="repeat_customer"]').prop('checked', false);
            $('input[name="liked_sample"]').prop('checked', false);
            $('textarea[name="liked_no_reason"]').val('');
            setTimeout(function() {
        ...
        case 'repeat1':
            $('section#finalRepeat').fadeOut();
            $('input[name="sampling_flavor_2"]').prop('checked', false);
            $('input[name="sampling_kit_2"]').prop('checked', false);
            setTimeout(function() {
                $('section#sampleFeedback').fadeIn();
            }, 800);
...
function identifyMatch() {
    var matchFound;
    var identityObject = {
        'first_name': $('#bluForm-name').val(),
        'last_name': $('#bluForm-lname').val(),
        'zip': $('#bluForm-zip').val(),
        'dob': reformattedDOB
    };

    identity = JSON.stringify(identityObject);

    console.log(identity);

    $.ajax({
        type: "POST",
        url: "http://blu.momentum.clong.mmahost5.networkninja.com/api/v1/identity",
        dataType: "json",
        async: false,
        data: {data : identity},
        success: function(result) {
